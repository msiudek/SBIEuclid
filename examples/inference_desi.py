"""
Stellar mass estimation for real Euclid galaxies matched to COSMOS-DESI.
...
python examples/inference_desi.py \
    --n-gal 500 --snr-min 3 --n-bands-min 3 --n-samples 200 \
    --outdir sbi-logs/inference_desi 2>&1 | tee sbi-logs/inference_desi.log

Selection criteria applied before inference:
  - valid z_desi spectroscopic redshift
  - valid logM_desi_Cigale reference mass
...
def parse_args():
    p = argparse.ArgumentParser(description="SBI mass inference on COSMOS-DESI matched catalog")
    ...
    p.add_argument("--n-bands-min", type=int,   default=3,
                   help="Minimum number of bands with SNR >= snr-min (default: 3)")
    p.add_argument("--n-samples",   type=int,   default=200,
                   help="Number of posterior samples per galaxy (default: 200)")
    p.add_argument("--outdir",      type=str,   default="sbi-logs/inference_desi",
...
def build_obs_array(flux_2d, fluxerr_2d, limits, snr_threshold=1.0, observation_space="flux"):
    """
...
def main():
    ...
    # Reference values from DESI/CIGALE
    z_ref    = np.array(cat["z_desi"],            dtype=float)
    mass_ref = np.array(cat["logM_desi_Cigale"],  dtype=float)   # log10(M/Msun)
    mass_err = np.array(cat["logM_err_desi_Cigale"], dtype=float)
    sfr_ref  = np.array(cat["logSFR_desi_Cigale"],   dtype=float)
    sfr_err  = np.array(cat["logSFR_err_desi_Cigale"], dtype=float)
    ...
    mass_err_sel = mass_err[sel]
    sfr_sel     = sfr_ref[sel]
    sfr_err_sel = sfr_err[sel]
    ...
    sx.infer_z    = False  # condition on catalog redshift
    ...
    # ------------------------------------------------------------------
    # 3. Build observation array (n_gal, 2*n_filt)
    # ------------------------------------------------------------------
    ...
    np.savez(result_file,
             ...
             logM_desi=mass_sel,
             logM_desi_err=mass_err_sel,
             logSFR_desi=sfr_sel,
             logSFR_desi_err=sfr_err_sel,
    ...
    print(f"  Median Δ(SBI-DESI)   = {np.median(delta):.3f} dex")
    ...
    print(f"  DESI/CIGALE logM range: [{mass_sel[valid].min():.2f}, {mass_sel[valid].max():.2f}]")
    ...
    if np.any(sfr_valid):
        ...
        print(f"  Median Δ(SBI-DESI)   = {np.median(sfr_delta):.3f} dex")
        ...
        print(f"  DESI/CIGALE logSFR range: [{sfr_sel[sfr_valid].min():.2f}, {sfr_sel[sfr_valid].max():.2f}]")

    # ------------------------------------------------------------------
    # 8. Plots
    # ------------------------------------------------------------------
    # Re-map DESI reference arrays to names expected by shared vplots utilities
