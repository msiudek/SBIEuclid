...
class sbipix():
    ...
    def _add_noise_nan_limit(self, mag_array, filter_idx):
        ...
        if use_mag_lognormal:
            ...
            # Out-of-domain: background-noise-limited formula.
            # σ_flux = σ_lim  →  σ_mag = (2.5/ln10) * σ_lim / |flux|
            # For very faint atlas galaxies this → large σ_mag (clips at sigma_mag_max=10),
            # meaning the band carries no information — physically correct for non-detections.
            if np.any(~in_domain):
                flux_ood = np.maximum(np.abs(flux_true[~in_domain]), 1e-12)
                sigma_mag_ood = (2.5 / np.log(10)) * sigma_lim / flux_ood
                sigma_mag_ood = np.clip(sigma_mag_ood, self.noise_sigma_floor,
                                        self.noise_sigma_mag_max)
                sigma_mag[~in_domain] = sigma_mag_ood
