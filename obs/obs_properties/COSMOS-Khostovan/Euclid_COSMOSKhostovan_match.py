...
print(f"{len(matched)} galaxies")
# =========================
# FILTERS
# =========================

# valid redshift
zmask = matched['redshift'] > 0

# patch selection
patch_mask = []
for v in matched['patch_id_list']:
    try:
        if isinstance(v, (list, np.ndarray)):
            patch_mask.append(98 in v)
        else:
            patch_mask.append(int(v) == 98)
    except:
        patch_mask.append(False)

patch_mask = np.array(patch_mask)

# apply
matched = matched[zmask & patch_mask]

print(f"{len(matched)} galaxies after filtering")
# =========================
# SAVE
# =========================
